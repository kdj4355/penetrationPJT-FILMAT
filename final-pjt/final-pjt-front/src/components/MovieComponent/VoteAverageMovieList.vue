<template>
  <div v-if="movies" id="movieCarousel2" class="carousel slide" data-bs-interval="false">
...
// 현재 슬라이드 인덱스
const currentSlideIndex = computed(() => store.voteAverageSlideIndex);
...
onMounted(async () => {
  ...
  const carouselElement = document.querySelector('#movieCarousel2');
  ...
  carouselElement.addEventListener('slid.bs.carousel', (event) => {
    store.setVoteAverageIndex(event.to);
  });
});
</script>
