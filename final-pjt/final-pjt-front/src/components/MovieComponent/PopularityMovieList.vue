<template>
  <div v-if="movies" id="movieCarousel1" class="carousel slide" data-bs-interval="false">
      ...
      <div class="carousel-item" v-for="(movieGroup, index) in movieGroups" :key="index" :class="{ active: index === currentSlideIndex }">
        <div class="d-flex justify-content-center">
          ...
          <MovieCard 
            ...
            :key="movie.id"
            :movie="movie"
            :top-ranking="index * 5 + movieIndex + 1"
            />
        </div>
      </div>
    </div>
...
// 현재 슬라이드 인덱스
const currentSlideIndex = computed(() => store.popularitySlideIndex);
...
onMounted(async () => {
  ...
  const carouselElement = document.querySelector('#movieCarousel1');
  ...
  carouselElement.addEventListener('slid.bs.carousel', (event) => {
    store.setPopularitySlideIndex(event.to);
  });
});
</script>
